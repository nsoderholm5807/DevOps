        ...
        for device in invResponse:
            if net['id'] == device['networkId']:
                if datetime.fromisoformat(device['claimedAt'])<checkDate:
                    break
                else: settingURL = f"https://api.meraki.com/api/v1/networks/{net['id']}/settings"
                ...
                if not passValue:
                    response += f"Network: {net['name']} needs LSP password set and was created after {checkDate.strftime("%B %d, %Y")}\n"
                    remediate = True
                    break
    response += "\n"
    if remediate == True:
        return response
    return ""
# print(lspAudit(orgs[1],headers=headers))
